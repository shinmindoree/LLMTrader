...
LLM 전략 생성 프록시 서버: Entra ID로 Azure OpenAI v1 Responses API에 접근하고, 클라이언트는 `POST /generate`로 전략 코드를 요청한다.

## 역할 및 API

- **역할**: SaaS 백엔드 또는 로컬 클라이언트가 호출하면, 프록시가 Entra ID 토큰으로 Azure OpenAI v1 Responses API를 호출해 전략 Python 코드를 생성해 반환한다.
- **API 스펙**:
...
| 이름 | 필수 | 설명 |
|------|------|------|
| `OPENAI_BASE_URL` | 예 | Azure OpenAI v1 base URL (예: `https://xxx.openai.azure.com/openai/v1/`). |
| `OPENAI_MODEL` | 예 | 배포(모델) 이름 (예: `gpt-5.4-pro`). |
...
docker run --rm -p 8000:8000 \
  -e OPENAI_BASE_URL=https://xxx.openai.azure.com/openai/v1/ \
  -e OPENAI_MODEL=gpt-5.4-pro \
  llmtrader-relay
```
