...
## 11. SaaS 관점: 프록시 접근 방식

서비스가 SaaS 형태일 때 **최종 사용자는 프록시에 직접 접근하지 않고**, SaaS 백엔드를 통해서만 전략 생성(프록시)을 사용하는 구조가 맞다.

### 11.1 권장 흐름

```
[사용자] → 로그인 → [SaaS 웹/앱] → [SaaS 백엔드(API)] → [프록시] → [Azure OpenAI]
                              ↑
                        사용자 인증·권한·쿼터 적용
```

- **사용자**: SaaS에 가입/로그인(이메일·비밀번호, OAuth, 또는 Entra ID 등). 세션 또는 JWT로 인증.
- **SaaS 프론트엔드**: 로그인된 사용자만 전략 생성 UI/API 호출 가능.
- **SaaS 백엔드**: 요청 시 사용자 식별(세션/JWT 검증) → 쿼터·권한 확인 → **백엔드가 프록시를 호출**(서버 간 인증). 생성된 전략은 사용자/계정에 연결해 DB 등에 저장.
- **프록시**: SaaS 백엔드만 호출하도록 제한. 백엔드→프록시 인증은 API Key 또는 M2M 토큰 사용.

### 11.2 프록시 접근 정책

| 구분 | 권장 방식 | 비고 |
|------|-----------|------|
| **누가 프록시를 호출하는가** | SaaS 백엔드만 | 클라이언트(브라우저/앱)는 프록시 URL·키를 알 필요 없음. |
| **백엔드 → 프록시 인증** | 서버용 API Key (또는 Azure M2M) | 환경변수/시크릿에 보관, 프록시가 헤더(e.g. `X-API-Key`) 검증. |
| **사용자 구분** | 백엔드가 요청 시 `user_id`(또는 `tenant_id`)를 헤더/본문에 포함 | 프록시는 로깅·쿼터용으로만 사용. 실제 권한은 SaaS 백엔드에서 검증. |
| **사용자 인증** | SaaS에서 담당 (OAuth2/OIDC, 이메일 로그인 등) | 프록시는 사용자 로그인을 처리하지 않음. |

### 11.3 이렇게 할 때의 이점

- **보안**: 프록시 URL·키가 클라이언트에 노출되지 않음. 사용자는 SaaS 계정으로만 접근.
- **쿼터/과금**: 백엔드에서 사용자별·플랜별 제한 적용 후 프록시 호출. 프록시는 선택적으로 user_id 기반 rate limit만 적용.
- **감사**: SaaS 백엔드에서 “누가, 언제, 어떤 전략 생성 요청을 했는지” 기록. 프록시는 요청 로그만 해도 됨.
- **상용 전환**: 나중에 플랜별 제한, 결제 연동, 팀/조직 단위 격리는 모두 SaaS 백엔드·DB에서 처리 가능.

### 11.4 구현 시 참고

- 상세 설계: `infra/docs/saas-proxy-access.md`
- 프록시는 “호출 주체 = SaaS 백엔드”만 허용하는 API Key(또는 동등한 서버 간 인증)로 보호하고, 사용자 인증·전략 저장·쿼터는 SaaS 백엔드와 DB에서 처리한다.
