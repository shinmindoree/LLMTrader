"""볼린저 밴드 하단 반등 롱 전략 - 1분봉 초단타.

가격이 볼린저 밴드 하단을 터치한 후 상승 반등할 때 롱 진입, 상단 도달 시 청산.
"""
...
class BollingerBandsBounceLongStrategy(Strategy):
    """볼린저 밴드 하단 반등 롱 전략 - 하단 터치 후 상승 반등 시 진입."""
    ...
    def __init__(
        ...
        self.indicator_config = {
            "BBANDS": {"period": self.period, "nbdevup": self.nbdevup, "nbdevdn": self.nbdevdn},
        }
    ...
    def on_bar(self, ctx: StrategyContext, bar: dict[str, Any]) -> None:
        ...
        if isinstance(bbands, dict):
            upper = float(bbands.get("upperband", math.nan))
            middle = float(bbands.get("middleband", math.nan))
            lower = float(bbands.get("lowerband", math.nan))
        else:
            return

        close = float(bar.get("bar_close", ctx.current_price))

        if not math.isfinite(upper) or not math.isfinite(middle) or not math.isfinite(lower):
            return

        if self.prev_lower is None:
        ...
        if ctx.position_size > 0 and not self.is_closing:
            if close >= upper:
                ...
                ctx.close_position(reason=f"Exit Long (BB upper)")
                self.prev_lower = lower
                self.prev_close = close
                return
        ...
        if ctx.position_size == 0:
            if close <= lower:
                self.touched_lower = True
            elif self.touched_lower and close > self.prev_close and close > lower:
                ctx.enter_long(reason=f"Entry Long (BB bounce)")
                self.touched_lower = False
