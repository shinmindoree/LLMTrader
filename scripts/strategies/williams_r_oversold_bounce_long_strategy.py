"""Williams %R 과매도 반등 롱 전략 - 1분봉 초단타.

Williams %R이 -80 이하에서 상승 반등할 때 롱 진입, -20 이상 도달 시 청산.
"""
...
class WilliamsROversoldBounceLongStrategy(Strategy):
    """Williams %R 과매도 반등 롱 전략 - %R이 -80 이하에서 상승 반등 시 진입."""
    ...
    def __init__(
        ...
        self.indicator_config = {
            "WILLR": {"period": self.period},
        }

    def initialize(self, ctx: StrategyContext) -> None:
        register_talib_indicator_all_outputs(ctx, "WILLR")
    ...
    def on_bar(self, ctx: StrategyContext, bar: dict[str, Any]) -> None:
        ...
        value = float(ctx.get_indicator("WILLR", period=self.period))
        ...
        if ctx.position_size > 0 and not self.is_closing:
            if crossed_above(self.prev_value, value, self.exit_level):
                ...
                ctx.close_position(reason=f"Exit Long (Williams %R {self.prev_value:.2f} -> {value:.2f})")
                self.prev_value = value
                return

        if ctx.position_size == 0:
            if self.prev_value <= self.oversold_level and value > self.oversold_level:
                ctx.enter_long(reason=f"Entry Long (Williams %R bounce {self.prev_value:.2f} -> {value:.2f})")
