...
> 작성일: 2025-02-10
> 현재 상태: MVP 이후 단계. 핵심 트레이딩(백테스트/라이브), LLM 전략 생성, Job 관리, Next.js UI 구현 완료.
...
- ✅ **v1 (구현 완료)**: 전략 빌더에서 빠른 백테스트 실행 → 결과를 AI 채팅에 주입 → AI가 메트릭 분석/약점 식별/개선안 제시 → 수동 반복 루프
- ✅ **v1 (구현 완료)**: 백테스트/라이브 폼에서 전략 파라미터(STRATEGY_PARAMS) 실시간 수정 가능
- 🔜 **v2**: "Auto-optimize" 자동 반복 모드 — 백테스트→AI분석→코드수정을 N회 자동 반복, 반복 이력 비교 테이블, 사용자가 원하는 시점에 중단/수락
...
- 🔜 **v2**: 빠른 백테스트 전용 Worker 분리 — `test-backtest-worker` Container App 신설, internal ingress, 독립 오토스케일 (min 0, max 20), 유휴 시 0으로 스케일다운하여 비용 절감

### 6-2. 데이터 파이프라인

- ✅ **v1 (구현 완료)**: Azure Cache for Redis (Basic C0) — 캔들 캐시 TTL 5분, 쿼터 카운트, Private Endpoint
- 🔜 **v2**: Redis Standard/Premium 클러스터 업그레이드 — 레플리카 기반 고가용성, 더 긴 TTL + LRU eviction으로 캐시 적중률 향상
...
- ✅ **v1 (구현 완료)**: 전략 빌더 워크스페이스에 미니 수익곡선 (SVG 경량 차트)
- 🔜 **v2**: 빠른 백테스트 결과에 풀 TradingView 차트 통합 — 캔들 + 매수/매도 마커 + 지표 오버레이
...
| 8 | 전략 자동 개선 파이프라인 | 높음 | 높음 | **v1 수동 루프 구현 완료**, v2 Auto-optimize |
| 9 | 전략 마켓플레이스 | 높음 | 높음 | 수익화 핵심 |
| 10 | 다중 거래소 지원 | 높음 | 높음 | 확장성 |
| 11 | 빠른 백테스트 Worker 분리 | 중간 | 중간 | v2, 사용자 1000+ 시 |
| 12 | Redis 클러스터 업그레이드 | 중간 | 낮음 | v2, 트래픽 증가 시 |
| 13 | 빠른 백테스트 TradingView 차트 | 중간 | 낮음 | v2, UX 강화 |
