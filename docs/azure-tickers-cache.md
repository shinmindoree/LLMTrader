...
1. Redis에 **45초 이내** 갱신된 캐시가 있으면 그걸 우선 사용
2. 없거나 일부 심볼만 있으면 **한 번의** `fapi/v1/ticker/24hr` 목록 조회로 보충

클라이언트는 기존처럼 WebSocket + 위 API를 사용하며, 마운트 시 REST를 바로 호출해 초기 공백을 줄입니다.
