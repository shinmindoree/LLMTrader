...
CMD ["sh", "-c", "uv run python scripts/run_live_trading.py scripts/strategies/macd_hist_immediate_entry_takeprofit_strategy.py --symbol ETHUSDT --leverage ${LEVERAGE:-10} --candle-interval ${CANDLE_INTERVAL:-1m} --max-position ${MAX_POSITION:-1.0} --strategy-params \"${STRATEGY_PARAMS:-}\" --indicator-config \"${INDICATOR_CONFIG:-}\" --log-interval ${LOG_INTERVAL:-0} --stoploss-cooldown-candles ${STOPLOSS_COOLDOWN_CANDLES:-50} --stop-loss-pct ${STOP_LOSS_PCT:-0.05} --daily-loss-limit 500.0 --max-consecutive-losses 0 --yes"]
